import sys
import pygame
from objects import load_images
...
class Editor:
    def __init__(self):
        ...
        self.name = "level_1.json"
        self.assets = {
            'blocks': load_images('terrain/blocks'),
            'bricks': load_images('terrain/bricks'),
            'spawners': list(pygame.transform.scale2x(img) for img in load_images('traps/trap_spawners')),
            'items': list(pygame.transform.scale2x(img) for img in load_images('items/spawners'))
        }
    ...
    def run(self):
        while True:
            ...
            self.tile_map.render(self.screen, offset=render_scroll)
            ...
            if self.on_grid:
                self.screen.blit(current_tile_img, (tile_pos[0] * self.tile_map.tile_size - self.scroll[0],
                                                    tile_pos[1] * self.tile_map.tile_size - self.scroll[1]))
            else:
                self.screen.blit(current_tile_img, m_pos)
